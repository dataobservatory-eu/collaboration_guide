...
For any external literature or data source, please, store the citation information in well-formatted, thematic `.bib`  files.
...
- The [regions](https://reprex.nl/software/regions/) package is an offspring of the Eurostat package on [rOpenGov](https://github.com/rOpenGov) and started off as a tool to validate and re-code regional Eurostat statistics. But its aim is to serve as a general solution for all sub-national statistics. Currently, the package keeps track of the boundary changes within the European Union for the period 1999-2021; stores generic vocabulary translation and joining functions for geographically coded data; performs imputation from higher to lower geographic levels (e.g. from NUTS2 to NUTS3) and vice versa (e.g. from NUTS3 to NUTS2); and aggregates or disaggregates the geographic data. It will be developed in parallel with other rOpenGov packages. 
...
They all are released on CRAN, and follow its guidelines for unit-testing and human review. As CRAN relies on extensive automated testing, the formatting standard is *very strict* both in terms of software code and documentation and the slightest deviation is likely to result in rejection.
...
We currently work with two platforms, and we must maintain compatibility with data repositories that our clients use. Both provide some validation, version control, and give a standard **doi** to our releases. ```WHAT ARE THE TWO PLATFORMS THAT WE USE?```

### Dataverse 

[dataverse.org/](https://dataverse.org/) has a well-supported API, and this is the choice of our first academic partner, IViR. ```PLEASE PROVIDE THE LINK TO IViR```
...
In support of its research programme CERN has developed tools for Big Data management and extended Digital Library capabilities for Open Data. Through Zenodo these Big Science tools could be effectively shared with the long-tail of research. ```HOW DOES ZENODO BENEFIT OUR PROJECT SPECIFICALLY? SECURE, RELIABLE AND WIDELY USED?```

## Publications

Our work is often embedded in publications. We aim to make the application of various formatting guidelines plain and straightforward. This is one of the main reasons for using markdown and Rmarkdown to document our work. Also, there are more and more conversion tools that automatically convert our long-form documentation from markdown or Rmarkdown to the formatting standards of nearly every scientific publisher.


## Namespace


### Naming variables, objects and features 

In our program code and databases, we are following Open Science naming guidelines. While there are many versions of naming guidelines, we pick something that works for all and adhere to it. 

For ease of programmatic use, we should follow a lowercase, snake-case variable naming. ```WHERE CAN WE FIND OFT-USED OPEN SCIENCE NAMING GUIDELINES? WHERE DO OUR GUIDELINES COME FROM?
